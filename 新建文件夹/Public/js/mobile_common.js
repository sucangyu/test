...
{                                                
    ...
    if($("#buy_goods_form").length > 0){        
        $.ajax({
            type : "POST",
            url:base_url+"/index.php?m=Home&c=Cart&a=ajaxAddCart",
            ...
            success: function(data){	
			   ...
			   if(to_catr == 1)  //直接购买
			   {
				   location.href = base_url+"/index.php?m=Mobile&c=Cart&a=cart";
			   }
        ...
        $.ajax({
            ...
            success: function(data){
				   ...
				   {
					    //layer.open({content: data.msg,time: 2});
						location.href =base_url+ "/index.php?m=Mobile&c=Goods&a=goodsInfo&id="+goods_id;
				   }
				   else
				   {  
					    if(data.status < 0)
